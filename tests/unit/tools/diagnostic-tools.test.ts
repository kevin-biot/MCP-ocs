...
describe('DiagnosticToolsV2', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(OcWrapperV2.prototype as any, 'executeOc').mockImplementation(async (..._a: any[]) => {
      const args = _a[0] as string[];
      if (args[0] === 'get' && args[1] === 'nodes') return { stdout: JSON.stringify({ items: [] }), stderr: '' };
      if (args[0] === 'get' && args[1] === 'clusteroperators') return { stdout: JSON.stringify({ items: [] }), stderr: '' };
      if (args[0] === 'get' && args[1] === 'pod') return { stdout: JSON.stringify({ metadata: { name: 'p1' }, status: { conditions: [] } }), stderr: '' };
      if (args[0] === 'get' && args[1] === 'namespaces') return { stdout: JSON.stringify({ items: [] }), stderr: '' };
  ...
  it('oc_diagnostic_namespace_health delegates to v2 handler and returns JSON', async () => {
    ...
    const out = await tool.execute({ sessionId: 's1', namespace: 'ns' }) as any;
  ...
  it('oc_diagnostic_pod_health returns pod health JSON', async () => {
    ...
    const out = await tool.execute({ sessionId: 's1', namespace: 'ns', podName: 'p1' }) as any;
    const obj = typeof out === 'string' ? JSON.parse(out) : out;
  ...
  it('oc_diagnostic_rca_checklist returns RCA report envelope', async () => {
    ...
    jest.spyOn((rcaModule as any).RCAChecklistEngine.prototype, 'executeRCAChecklist').mockResolvedValue({
      reportId: 'rep-1',
    ...
    const out = await tool.execute({ sessionId: 's1', namespace: 'ns', outputFormat: 'json' }) as any;
    const obj = typeof out === 'string' ? JSON.parse(out) : out;
    expect(typeof obj).toBe('object');
