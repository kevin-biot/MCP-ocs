...
const createOcWrapperMock = () => ({
  getPVCs: jest.fn(async (ns: string) => ({ items: [student03PendingPVCScenario.pvc] })),
  getEvents: jest.fn(async (ns: string) => ({ items: eventDataForPendingPVCs })),
...
describe('PVCBindingRCAEngine', () => {
  let oc: ReturnType<typeof createOcWrapperMock>;
  let memory: ReturnType<typeof createMemoryMock>;
  let engine: PVCBindingRCAEngine;
  ...
  beforeEach(() => {
    ...
    engine = new PVCBindingRCAEngine(oc as any, memory as any);
  });
  ...
  it('should analyze evidence and weight correctly', async () => {
    ...
    const avgWeight = parsed.rootCause.evidence.reduce((a: number, e: any) => a + e.weight, 0) / parsed.rootCause.evidence.length;
  ...
  it('should integrate through StorageIntelligenceTools execute()', async () => {
    const tools = new StorageIntelligenceTools(oc as any, memory as any);
