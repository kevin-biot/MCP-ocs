...
} from '../../../src/lib/tools/tool-registry';

type StandardToolLike = any; // relax typing for negative validation tests
...
describe('UnifiedToolRegistry', () => {
  ...
  function makeTool(overrides: Partial<StandardToolLike> = {}): StandardToolLike {
    return {
      name: 'test_tool',
  ...
  it('validates required fields and categories/versions', () => {
    // Missing fullName
    const missing: StandardToolLike = { ...makeTool(), fullName: undefined };
    expect(() => registry.registerTool(missing)).toThrow(/missing required field 'fullName'/);

    // Invalid category
    const badCat: StandardToolLike = { ...makeTool(), name: 'bad_cat', fullName: 'ns_bad_cat', category: 'nope' };
    expect(() => registry.registerTool(badCat)).toThrow(/invalid category/);

    // Invalid version
    const badVer: StandardToolLike = { ...makeTool(), name: 'bad_ver', fullName: 'ns_bad_ver', version: 'v3' };
