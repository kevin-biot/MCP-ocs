...
jest.mock('util', () => {
  let calls = 0;
  return {
    promisify: (_fn: any) => (cmd: string, _opts: any) => {
      calls++;
      (globalThis as any).__oc_calls = calls;
      const mode = (globalThis as any).__oc_mode;
      if (mode === 'timeout') {
      ...
      const out = (globalThis as any).__oc_stdout || '{"items":[]}';
...
describe('OcWrapperV2', () => {
  ...
  beforeEach(() => {
    (globalThis as any).__oc_mode = undefined;
    (globalThis as any).__oc_stdout = undefined;
    (globalThis as any).__oc_calls = 0;
  ...
  it('executes and returns OcResult with caching behavior', async () => {
    (globalThis as any).__oc_stdout = '{"items":[]}';
    ...
    expect((globalThis as any).__oc_calls).toBe(1);
  });
  ...
  it('handles JSON parse errors from getters', async () => {
    (globalThis as any).__oc_stdout = 'not-json';
  ...
  it('gracefully falls back for getRoutes/getIngress when not available', async () => {
    (globalThis as any).__oc_mode = 'error';
  ...
  it('throws timeout and generic errors from executeOc', async () => {
    (globalThis as any).__oc_mode = 'timeout';
  ...
  it('validates args and namespace formatting', async () => {
    await expect(oc.executeOc([] as any)).rejects.toThrow(/No arguments provided/);
    await expect(oc.executeOc(['get;rm -rf /'] as any)).rejects.toThrow(/Invalid characters/);
