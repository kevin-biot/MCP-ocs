...
# ENHANCED MODE - Structured analysis for: tests/unit/config/schema.test.ts
echo "🤖 ENHANCED MODE: tests/unit/config/schema.test.ts"
echo "============================================="
echo

# Capture test output for analysis
echo "Running test and analyzing output..."
test_output=$(npm run test:unit -- "tests/unit/config/schema.test.ts" --verbose --no-coverage 2>&1)
test_result=$?
...
echo ""
echo "🎯 SPECIFIC ISSUES FOR: schema.test.ts"
echo "==========================================="
...
if [[ $import_errors -gt 0 ]]; then
    echo "3. Check import paths in tests/unit/config/schema.test.ts"
    echo "4. Verify required dependencies are installed"
fi

if [[ $mock_errors -gt 0 ]]; then
    echo "5. Fix Jest mock setup in tests/unit/config/schema.test.ts"
fi
...
echo "2. Re-run raw mode: scripts/test/dual-mode/raw-config-schema.sh"
echo "3. Re-run enhanced mode: scripts/test/dual-mode/enhanced-config-schema.sh"
