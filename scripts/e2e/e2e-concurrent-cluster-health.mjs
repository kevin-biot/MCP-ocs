  ...
  return new Promise((resolve, reject)=>{
    ...
    const onData=(chunk)=>{
      ...
      let idx; while((idx=buf.indexOf('\n'))!==-1){
        ...
        if(!line.trim()) continue; try{ const msg=JSON.parse(line); if(msg.id===id){ clearTimeout(t); proc.stdout.off('data',onData); resolve(msg); return; } }catch{}
      }
    };
...
async function run(){
  ...
  const session = `e2e-concurrent-${Date.now()}`;
  ...
  const r3 = await waitFor(proc, 3, 60000);
  const r4 = await waitFor(proc, 4, 60000);
  try { proc.kill(); } catch {}
  const parse = (r)=>{ try { return JSON.parse(r?.result?.content?.[0]?.text || ''); } catch { return null; } };
  const a = parse(r3), b = parse(r4);
  const pa = !!a?.evidence?.pass, pb = !!b?.evidence?.pass;
  console.error(`Concurrent results: A=${pa} B=${pb}`);
  console.log(JSON.stringify({ A: a?.evidence, B: b?.evidence }));
}

run().catch(e=>{ console.error('Concurrent E2E failed:', e?.message||e); process.exit(1); });
