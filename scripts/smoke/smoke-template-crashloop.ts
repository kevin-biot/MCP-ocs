...
async function execTool(name: string, params: any){ if(name==='oc_read_logs') return { logs:{ tail:'...logs.tail...' } }; return { ok:true, tool:name, params }; }

async function main(){
  const sessionId='smoke-te-crash';
  const reg=new TemplateRegistry(); await reg.load();
  const sel=reg.selectByTarget('crashloopbackoff'); if(!sel){console.error('no template');process.exit(1)}
  const engine=new TemplateEngine();
  const plan=engine.buildPlan(sel.template,{sessionId, bounded:true, stepBudget:3, vars:{ ns:'ns-a', selector:'app=x', pod:'pod-x', container:'c1' }});
