...
async function main(){
  const sessionId='smoke-te-pvc';
  const reg=new TemplateRegistry(); await reg.load();
  const sel=reg.selectByTarget('pvc-binding'); if(!sel){console.error('no template');process.exit(1)}
  const engine=new TemplateEngine();
  const plan=engine.buildPlan(sel.template,{sessionId, bounded:true, stepBudget:3, vars:{ ns:'ns-a', pvc:'pvc-a', sc:'standard' }});
