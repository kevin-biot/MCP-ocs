...
async function main(){
  const sessionId='smoke-te-sched';
  const reg=new TemplateRegistry(); await reg.load();
  const sel=reg.selectByTarget('scheduling-failures'); if(!sel){console.error('no template');process.exit(1)}
  const engine=new TemplateEngine();
  const plan=engine.buildPlan(sel.template,{sessionId, bounded:true, stepBudget:3, vars:{ ns:'openshift-ingress', pendingPod:'router-pending', nodeFromEvent:'node-a' }});
