...
export class EnhancedSequentialThinkingOrchestrator {
  ...
  private includesSchedulingRedFlags(text: string): boolean {
    const s = text.toLowerCase();
    return /faileddcheduling/.test(s) || /taint/.test(s) || /anti-?affinity/.test(s);
  }

  private extractNodeFromText(text: string): string | undefined {
  ...
  private async storeMiniPlanStrategy(sessionId: string, steps: ToolStep[], telemetry?: Record<string, any>): Promise<void> {
    try {
