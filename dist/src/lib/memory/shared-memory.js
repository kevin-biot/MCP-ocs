...
export class SharedMemoryManager {
    ...
    constructor(config) {
        ...
        if (FEATURE_FLAGS.UNIFIED_MEMORY) {
            ...
            if (typeof config.chromaHost === 'string')
                uniCfg.chromaHost = config.chromaHost;
            if (typeof config.chromaPort === 'number')
                uniCfg.chromaPort = config.chromaPort;
    ...
    async storeConversation(memory) {
        ...
        if (this.unified) {
            try {
                ...
                if (FEATURE_FLAGS.MEMORY_STRUCTURED_LOGS)
                ...
                if (FEATURE_FLAGS.MEMORY_STRUCTURED_LOGS)
                    console.error(JSON.stringify({ subsystem: 'memory', event: 'store_conversation', backend: 'unified', sessionId: memory.sessionId, timestamp: memory.timestamp, status: 'fail', message: String(error?.message || error) }));
            }
            return jsonId;
        }
    ...
    async storeOperational(memory) {
        ...
        if (this.unified) {
            try {
                await this.unified.storeOperational(memory);
                if (FEATURE_FLAGS.MEMORY_STRUCTURED_LOGS)
                ...
                if (FEATURE_FLAGS.MEMORY_STRUCTURED_LOGS)
                    console.error(JSON.stringify({ subsystem: 'memory', event: 'store_operational', backend: 'unified', incidentId: memory.incidentId, timestamp: memory.timestamp, status: 'fail', message: String(error?.message || error) }));
            }
            return jsonId;
        }
        // Store in ChromaDB if available
    ...
    async _runExclusive(fn) {
        ...
        this._mutexQueue = new Promise(res => (resolvePrev = res));
        await prev.catch(() => { });
        try {
    ...
    async searchConversations(query, limit = 5) {
        if (this.unified) {
            try {
                ...
                if (FEATURE_FLAGS.MEMORY_STRUCTURED_LOGS)
                    console.error(JSON.stringify({ subsystem: 'memory', event: 'search_conversations', backend: 'unified', query, status: 'fail', message: String(error?.message || error) }));
            }
        }
        if (this.chromaClient.isChromaAvailable()) {
            try {
                ...
                if (FEATURE_FLAGS.MEMORY_STRUCTURED_LOGS)
                    console.error(JSON.stringify({ subsystem: 'memory', event: 'search_conversations', backend: 'chroma', query, status: 'fail', message: String(error?.message || error) }));
            }
        }
    ...
    async searchOperational(query, limit = 5) {
        if (this.unified) {
            try {
                ...
                if (FEATURE_FLAGS.MEMORY_STRUCTURED_LOGS)
                    console.error(JSON.stringify({ subsystem: 'memory', event: 'search_operational', backend: 'unified', query, status: 'fail', message: String(error?.message || error) }));
            }
        }
        if (this.chromaClient.isChromaAvailable()) {
            try {
                ...
                if (FEATURE_FLAGS.MEMORY_STRUCTURED_LOGS)
                    console.error(JSON.stringify({ subsystem: 'memory', event: 'search_operational', backend: 'chroma', query, status: 'fail', message: String(error?.message || error) }));
            }
        }
        // Fallback to JSON text search
