...
export function evaluateWeighted(r, inputs) {
    const breakdown = {};
    let score = 0;
    ...
    if (r.bands) {
        ...
        for (const [band, raw] of Object.entries(r.bands)) {
            ...
            if (condition.toLowerCase() === 'otherwise') {
                label = label ?? band;
                continue;
            }
...
export function evaluateGuards(r, inputs) {
    const failing = [];
...
export function evaluateRubrics(rubrics, inputs) {
    const out = {};
    if (!rubrics)
        return out;
    for (const [key, r] of Object.entries(rubrics)) {
        const rb = r;
        if (rb.kind === 'weighted')
            out[key] = evaluateWeighted(rb, inputs);
        else if (rb.kind === 'guards')
            out[key] = evaluateGuards(rb, inputs);
        else if (rb.kind === 'mapping')
            out[key] = evaluateMapping(rb, inputs);
    }
    return out;
}
...
function normalizeBandExpr(exprStr) {
