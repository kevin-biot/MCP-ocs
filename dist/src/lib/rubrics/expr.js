    ...
    while (i < src.length) {
        ...
        if (isAlpha(c)) {
            ...
            if (/^(true|false)$/i.test(word))
                tokens.push({ type: 'bool', value: word.toLowerCase() });
            else
...
export class ExprEvaluator {
    ...
    evaluate(expression, context) {
        ...
        if (!exp || exp.toLowerCase() === 'otherwise')
            return true; // treat as fallback true
    ...
    eat() { return this.tokens[this.pos++] || { type: 'eof' }; }
    expect(type) { if (this.current().type !== type)
        throw new Error(`expr: expected ${type}`); this.eat(); }
    // or := and ('||' and)*
    parseOr() {
...
export function normalize(value, spec) {
    if (!spec)
        return value;
