...
export class EnhancedNamespaceHealthChecker {
    ...
    constructor(ocWrapper, memoryManager, infrastructureChecker) {
        this.ocWrapper = ocWrapper;
        this.memoryManager = memoryManager;
        this.infrastructureChecker = infrastructureChecker;
    ...
    async checkHealth(input) {
        ...
        try {
            ...
            let infrastructureCorrelation = { enabled: false, enhancedSuspicions: baseSuspicions };
            ...
            const human = this.generateEnhancedHumanSummary(namespace, status, podHealth, pvcHealth, routeHealth, infrastructureCorrelation.enhancedSuspicions, scaleDownAnalysis, infrastructureCorrelation.rootCauseCandidate);
            const result = {
                namespace,
                status,
                checks: {
            ...
            return this.createFailureResult(namespace, `Health check failed: ${error instanceof Error ? error.message : 'Unknown error'}`, startTime);
        }
    }
    /**
     ...
     */
    shouldRunInfrastructureCorrelation(podHealth, pvcHealth) {
        return (podHealth.pending.length > 0 || // Pending pods might be zone/node issues
     ...
     */
    enhanceSuspicionsWithInfrastructure(baseSuspicions, infraAnalysis) {
        ...
        if (infraAnalysis.summary.hasInfrastructureIssues) {
            ...
            for (const conflict of infraAnalysis.zoneAnalysis.zoneConflicts) {
                if (conflict.conflictSeverity === 'critical') {
                    enhancedSuspicions.unshift(`🚨 ZONE CONFLICT: PV ${conflict.pvName} requires zone ${conflict.requiredZone} (unavailable) - this explains pending pods`);
                }
            }
            ...
            if (unavailableZones.length > 0) {
                enhancedSuspicions.unshift(`⚠️ ZONE UNAVAILABLE: ${unavailableZones.length} zone(s) scaled to 0 replicas: ${unavailableZones.map(z => z.zone).join(', ')}`);
            }
        }
     ...
     */
    identifyRootCauseCandidate(infraAnalysis) {
     ...
     */
    generateEnhancedHumanSummary(namespace, status, pods, pvcs, routes, suspicions, scaleDownAnalysis, rootCauseCandidate) {
        const parts = [];
    ...
    analyzePods(podsData) {
        ...
        for (const pod of pods) {
            ...
            const isReady = containerStatuses.every((container) => container.ready === true);
    ...
    async analyzeRoutes(routesData, includeConnectivityTest) {
    ...
    analyzeScaleDownPatterns(deploymentsData, eventsData, podHealth) {
    ...
    generateSuspicions(pods, pvcs, routes, events, scaleDownAnalysis) {
        const suspicions = [];
        if (pods.crashloops.length > 0) {
    ...
    determineOverallStatus(pods, pvcs, routes, events, scaleDownAnalysis) {
    ...
    // Helper methods
    createFailureResult(namespace, error, startTime) {
        return {
            namespace,
            status: 'failing',
            checks: {
    ...
    async testRouteConnectivity(url) {
        try {
