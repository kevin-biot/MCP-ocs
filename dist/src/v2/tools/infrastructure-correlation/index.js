...
export class InfrastructureCorrelationChecker {
     ...
     */
    async checkInfrastructureCorrelation(input) {
        ...
        try {
            ...
            const storageConflicts = await this.detectStorageZoneConflicts(input, infrastructureData, zoneAnalysis);
            // 5. Generate intelligent recommendations
            const recommendations = await this.generateRecommendations(zoneAnalysis, storageConflicts, memoryContext);
     ...
     */
    async searchInfrastructureMemory(input) {
        const searchQueries = [
     ...
     */
    async gatherInfrastructureData(input) {
     ...
     */
    async analyzeZoneAvailability(infrastructureData) {
     ...
     */
    async detectStorageZoneConflicts(input, infrastructureData, zoneAnalysis) {
        const conflicts = [];
     ...
     */
    async generateRecommendations(zoneAnalysis, storageConflicts, memoryContext) {
        ...
        for (const conflict of storageConflicts.filter(c => c.conflictSeverity === 'critical')) {
            recommendations.push(`🚨 CRITICAL: Scale up MachineSet in zone ${conflict.requiredZone} to resolve PV ${conflict.pvName} conflict`);
        }
        // Priority 2: Address zone availability issues
        for (const zone of zoneAnalysis.filter(z => z.status === 'unavailable')) {
            recommendations.push(`⚠️  Zone ${zone.zone} is unavailable (0 replicas) - consider scaling up if workloads require this zone`);
        }
        ...
        if (memoryContext.length > 0) {
            ...
            if (historicalResolutions.length > 0) {
                recommendations.push(`💡 Based on similar incidents: ${historicalResolutions[0]}`);
            }
        }
    ...
    async getPersistentVolumeAnalysis(namespace) {
        try {
    ...
    async findPodsUsingPV(pv, namespace) {
        try {
    ...
    calculateConfidenceScore(zoneAnalysis, storageConflicts, memoryContext) {
    ...
    deduplicateMemoryResults(results) {
        const seen = new Set();
     ...
     */
    async storeInfrastructureMemory(sessionId, input, result) {
        try {
