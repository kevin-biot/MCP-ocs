...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (ENABLE_TEMPLATE_ENGINE && request.params.arguments?.triageTarget) {
        const targs = request.params.arguments;
        ...
        if (!sel) {
            ...
            const plan = templateEngine.buildPlan(sel.template, { sessionId, bounded, stepBudget, vars: args?.vars || {} });
            const enforcerConfig = { maxSteps: plan.boundaries.maxSteps, timeoutMs: plan.boundaries.timeoutMs };
            if (targs.allowedNamespaces)
                enforcerConfig.allowedNamespaces = targs.allowedNamespaces;
            const enforcer = new BoundaryEnforcer(enforcerConfig);
            const steps = enforcer.filterSteps(plan.steps);
            const exec = [];
            const ctxVars = { ...(targs.vars || {}) };
            const replaceVarsLocal = (obj) => {
                if (obj == null)
                    return obj;
                ...
                if (Array.isArray(obj))
                    return obj.map(replaceVarsLocal);
                if (typeof obj === 'object') {
                    const out = {};
                    for (const [k, v] of Object.entries(obj))
                        out[k] = replaceVarsLocal(v);
                    return out;
                }
                return obj;
            };
            ...
            for (const s of steps) {
                ...
                try {
                    if (s.tool === 'oc_read_describe' && params?.resourceType === 'endpoints' && isPlaceholder(params?.name)) {
                        ...
                        if (!ctxVars.service && routeName && ns) {
                            ...
                            if (m) {
                                ctxVars.service = m[1];
                                params = { ...params, name: ctxVars.service };
                            }
                        }
                    }
                        ...
                        if (!ctxVars.backendPod && ctxVars.service && ns) {
                            ...
                            if (selector) {
                                ...
                                try {
                                    ...
                                    const ready = list.find((p) => String(p?.ready || '') === '1/1') || list[0];
                                    if (ready?.name) {
                                        ctxVars.backendPod = ready.name;
                                        params = { ...params, name: ctxVars.backendPod };
                                    }
                                }
                                catch { }
                            }
                    ...
                    if (s.tool === 'oc_read_describe' && params?.resourceType === 'pod') {
                        ...
                        try {
                            ...
                            if (m && !ctxVars.nodeFromEvent)
                                ctxVars.nodeFromEvent = m[1];
                        }
                        catch { }
                        if (!ctxVars.nodeFromEvent) {
                            try {
                                ...
                                const pick = list.find((n) => !n.ready) || list[0];
                                if (pick?.name)
                                    ctxVars.nodeFromEvent = pick.name;
                            }
                            catch { }
                        }
                    }
                ...
                try {
                    if (s.tool === 'oc_read_get_pods' && String(params?.namespace) === 'openshift-ingress') {
                        ...
                        const pending = pods.find((p) => String(p?.name || '').startsWith('router-default-') && (p?.status === 'Pending' || String(p?.ready || '') === '0/1'));
                        if (pending?.name)
                            ctxVars.pendingRouterPod = pending.name;
                        // Preflight fallback: if a provided pendingRouterPod is not present, select any router-default-*
                        try {
                            const routers = pods.filter((p) => String(p?.name || '').startsWith('router-default-'));
                            if (ctxVars.pendingRouterPod && !routers.some((p) => p?.name === ctxVars.pendingRouterPod) && routers[0]?.name) {
                    ...
                    if (s.tool === 'oc_read_get_pods') {
                        const parsed = typeof r === 'string' ? JSON.parse(r) : r;
                        const pods = Array.isArray(parsed?.pods) ? parsed.pods : (Array.isArray(parsed?.items) ? parsed.items.map((it) => ({ name: it?.metadata?.name, status: it?.status?.phase })) : []);
                        const crash = pods.find((p) => /crashloopbackoff/i.test(String(p?.status || '')));
                        const anyPod = pods[0];
                        if (!ctxVars.pod && (crash?.name || anyPod?.name))
                            ctxVars.pod = crash?.name || anyPod?.name;
                    }
                }
                catch { }
            }
            ...
            let diagnosis_status = undefined;
            if (ENABLE_RUBRICS) {
                try {
                    ...
                    try {
                        ...
                        if (isInfra) {
                            ...
                            const resultsText = (exec || []).map((e) => typeof e?.result === 'string' ? e.result : JSON.stringify(e.result)).join('\n');
                            const scan = (p) => {
                                for (const e of exec || []) {
                                    try {
                                        const o = typeof e?.result === 'string' ? JSON.parse(e.result) : e?.result;
                                        if (o && typeof o === 'object' && typeof o[p] !== 'undefined')
                                            return o[p];
                                    }
                                    catch { /* ignore */ }
                                }
                            ...
                            const capacityUtil = Number((scan('capacity') || {}).utilization);
                            const violations = (resultsText.match(/FailedScheduling|No nodes fit/gi) || []).length;
                            const conds = (scan('conditions') || {});
                    ...
                    try {
                        ...
                        if (key.includes('cluster-health')) {
                            ...
                            const metaRes = evaluateRubrics({ metaDispatch: META_DISPATCH_CONFIDENCE_V1 }, { ingressSignal, pvcSignal, churnSignal });
                            rubrics.meta = metaRes;
                        }
                    }
                    catch { }
                    ...
                    try {
                        ...
                        const slo = String(summary?.slo?.label || 'N/A');
                    ...
                    try {
                        if (process.env.ENABLE_SHADOW_SEQ === 'true' && ENABLE_SEQUENTIAL_THINKING) {
                            const shadow = await sequentialThinkingOrchestrator.handleUserRequest(`Shadow constrained triage for ${triageTarget}`, sessionId, { bounded: true, mode: 'planOnly', triageTarget, stepBudget: Math.min(3, steps.length) });
    ...
    try {
        ...
        if (ENABLE_SEQUENTIAL_THINKING && args?.userInput) {
            const thinking = await sequentialThinkingOrchestrator.handleUserRequest(args.userInput, args.sessionId || `session-${nowEpoch()}`);
            ...
            const sessionId = args?.sessionId || `session-${nowEpoch()}`;
        ...
        try {
            if (name === 'oc_diagnostic_cluster_health' && typeof result === 'string') {
                ...
                if (mode === 'bounded' && pending > 0) {
                    ...
                    try {
                        await sharedMemory.storeOperational({
                            incidentId: `shim-engagement-${args?.sessionId || 'unknown'}-${nowEpoch()}`,
                            domain: 'cluster',
                            timestamp: nowEpoch(),
                    ...
                    const sid = String(args?.sessionId || `session-${nowEpoch()}`);
                    try {
        ...
        const sessionId = args?.sessionId || `auto-session-${nowEpoch()}`;
    ...
    const contents = [{ type: 'text', text: result }];
    try {
        if (ENABLE_RUBRICS) {
            const diag = computeDiagnosticRubricsIfAny(name, result);
            if (diag)
                contents.push({ type: 'text', text: JSON.stringify({ tool: name, rubrics: diag.rubrics, inputs: diag.inputs, summary: diag.summary }, null, 2) });
        }
    }
    catch { }
...
function computeCoreRubricInputs(target, evidence) {
...
async function buildSummaryCard(rubrics, evidence, rubricInputs, templateMeta) {
    ...
    const base = scrub({
        ...
        evidence: { completeness, minThreshold: Number(templateMeta.evidenceThreshold ?? 0), missing: Array.isArray(evidence?.missing) ? evidence.missing : undefined, present: Array.isArray(evidence?.present) ? evidence.present : undefined },
        rubricInputs: rubricInputs
    });
    try {
        const sloLabel = rubrics?.slo?.label;
        if (sloLabel) {
            base.slo = { label: sloLabel, why: rubrics?.slo?.matched };
            base.sloHint = computeSloHint(String(sloLabel));
        }
    }
    catch { }
    // Attach infrastructure rubric summaries (visual-only; not gated)
    try {
        const infra = rubrics?.infra;
        if (infra) {
            const badge = {
            ...
            base.infra = { ...badge, storageAffinity: infra?.storageAffinity?.label };
        }
    }
    catch { }
    // Attach memory recall (advisory)
    try {
        const recall = await tryRecallForTarget(String(base?.templateId || ''), base);
        if (recall) {
            ...
            try {
                ...
                const rr = evaluateRubrics({ recallConfidence: MEMORY_RECALL_CONFIDENCE_V1 }, recallInputs);
                recall.confidence = rr?.recallConfidence?.label;
            }
            catch { }
            base.recall = recall;
        }
...
async function tryRecallForTarget(targetId, summary) {
    try {
        // Basic heuristic: search by target id or priority label
        const query = `${targetId} ${String(summary?.priority?.label || '')}`.trim();
        const hits = await sharedMemory?.searchOperational?.(query, 5) || [];
        if (!Array.isArray(hits) || hits.length === 0)
            return undefined;
        const top = hits[0];
        const top1Similarity = Number(top?.similarity ?? top?.score ?? 0);
        let freshnessMin = undefined;
        const ts = top?.timestamp || top?.ts;
        if (typeof ts === 'number')
            freshnessMin = Math.floor((nowEpoch() - ts) / (1000 * 60));
        else if (typeof ts === 'string') {
            const d = Date.parse(ts);
            if (!isNaN(d))
                freshnessMin = Math.floor((nowEpoch() - d) / (1000 * 60));
        }
...
function persistSummary(planId, target, summary) {
    try {
...
function buildGuardsDetail(safetyRubric, inputs) {
    const out = [];
    const guards = Array.isArray(safetyRubric?.guards) ? safetyRubric.guards : [];
    for (const g of guards) {
        const m = String(g).match(/^\s*([A-Za-z_][A-Za-z0-9_]*)\s*(==|!=|>=|<=|>|<)\s*(true|false|[0-9.]+)\s*$/);
        const key = m?.[1];
        const op = m?.[2];
        const rawThresh = m?.[3];
        const observed = typeof key !== 'undefined' ? inputs[key] : undefined;
        let threshold = rawThresh;
        if (typeof rawThresh === 'string' && /^(true|false)$/i.test(rawThresh))
            threshold = rawThresh.toLowerCase() === 'true';
        else if (typeof rawThresh === 'string' && /[0-9.]+/.test(rawThresh))
...
function sloInputsForTarget(t) {
    const base = { ingressPending: 0, route5xx: 0, pvcBinding: 0, crashloop: 0, apiDegraded: 0 };
    const key = String(t || '').toLowerCase();
    if (key.includes('ingress'))
        return { ...base, ingressPending: 1 };
    if (key.includes('route'))
        return { ...base, route5xx: 1 };
    if (key.includes('pvc'))
        return { ...base, pvcBinding: 1 };
    if (key.includes('crash'))
        return { ...base, crashloop: 1 };
    if (key.includes('api'))
        return { ...base, apiDegraded: 1 };
    return base;
}
function computeDiagnosticRubricsIfAny(toolName, resultText) {
    let obj;
    try {
    ...
    if (/oc_diagnostic_cluster_health/i.test(toolName)) {
        try {
            ...
            const degradedList = Array.isArray(obj?.operators?.degraded) ? obj.operators.degraded : [];
            inputs.operatorsDegraded = degradedList.length;
            inputs.etcdHealthy = !degradedList.some((n) => /etcd/i.test(String(n)));
            ...
            if (rubrics)
                summary = buildCompactSummary(rubrics);
        }
        catch { }
    }
    else if (/oc_diagnostic_pod_health/i.test(toolName)) {
        try {
            const issues = Array.isArray(obj?.health?.issues) ? obj.health.issues : [];
        ...
        try {
            ...
            const checks = Array.isArray(res?.checksPerformed) ? res.checksPerformed : [];
        ...
        try {
            ...
            if (typeof ts === 'number')
                days = Math.abs(nowEpoch() - ts) / (1000 * 60 * 60 * 24);
            else if (typeof ts === 'string') {
                const d = Date.parse(ts);
                if (!isNaN(d))
                    days = Math.abs(nowEpoch() - d) / (1000 * 60 * 60 * 24);
            }
    ...
    if (rubrics)
        return { inputs, rubrics, summary };
    return null;
}
function buildCompactSummary(rubrics) {
    return {
...
function persistShadowDiff(planId, target, data) {
    try {
        const dir = path.join('logs', 'runs');
        fs.mkdirSync(dir, { recursive: true });
...
function buildShadowDiff(templateOut, shadowRes) {
    const out = { steps: {}, rubrics: {} };
    try {
        const shadowSteps = Array.isArray(shadowRes?.toolStrategy?.steps) ? shadowRes.toolStrategy.steps.map((s) => s.tool) : [];
        out.steps = {
