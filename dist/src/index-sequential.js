...
async function tryRecallForTarget(targetId, summary) {
    try {
        ...
        if (typeof ts === 'number')
            freshnessMin = Math.floor((Date.now() - ts) / (1000 * 60));
        else if (typeof ts === 'string') {
            const d = Date.parse(ts);
            if (!isNaN(d))
                freshnessMin = Math.floor((Date.now() - d) / (1000 * 60));
        }
...
function computeDiagnosticRubricsIfAny(toolName, resultText) {
    ...
    if (/oc_diagnostic_cluster_health/i.test(toolName)) {
        ...
        try {
            ...
            if (typeof ts === 'number')
                days = Math.abs(Date.now() - ts) / (1000 * 60 * 60 * 24);
            else if (typeof ts === 'string') {
                const d = Date.parse(ts);
                if (!isNaN(d))
                    days = Math.abs(Date.now() - d) / (1000 * 60 * 60 * 24);
            }
