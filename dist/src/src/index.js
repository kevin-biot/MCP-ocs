...
try {
    ...
    console.error('Shared memory initialization failed:', e instanceof Error ? e.message : e);
}
...
toolRegistry.registerTool({
    ...
    async execute(args) {
        ...
        const triageTarget = typeof raw.triageTarget === 'string' ? raw.triageTarget : undefined;
        ...
        let mode = typeof raw.mode === 'string' ? raw.mode : (firstStepOnly ? 'firstStepOnly' : (bounded ? 'firstStepOnly' : 'planOnly'));
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (ENABLE_TEMPLATE_ENGINE && request.params.arguments?.triageTarget) {
        ...
        if (!sel) {
            ...
            const content = { content: [{ type: 'text', text: JSON.stringify(out, null, 2) }] };
            return content;
        }
    }
    ...
    try {
        ...
        const contents = [{ type: 'text', text: JSON.stringify({ success: false, tool: name, error: serialized, timestamp: new Date().toISOString() }, null, 2) }];
...
function buildSummaryCard(rubrics, evidence, rubricInputs, templateMeta) {
