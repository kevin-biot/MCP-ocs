...
export class RCAChecklistEngine {
    ...
    async checkStorageHealth(result, namespace) {
        ...
        try {
            ...
            const defaultSC = storageClasses.items.find((sc) => sc.metadata.annotations?.['storageclass.kubernetes.io/is-default-class'] === 'true');
     ...
     */
    async checkCriticalNamespaces(result) {
    ...
    analyzeNodeHealth(nodes) {
        ...
        for (const node of nodes.items) {
            ...
            for (const condType of concerningConditions) {
                const condition = conditions.find((c) => c.type === condType);
            ...
            if (status?.hard && status?.used) {
                ...
                for (const [resource, hardLimit] of Object.entries(status.hard)) {
                    ...
                    if (used && hardLimit) {
                        ...
                        const usedNum = this.parseResourceValue(used);
                        const hardNum = this.parseResourceValue(hardLimit);
    ...
    analyzeChecklistResults(result) {
