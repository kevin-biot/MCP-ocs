...
describe('Basic Test Setup', () => {
    ...
    it('should have test utilities available', () => {
        ...
        expect(globalThis.testUtils.createTestConfig).toBeDefined();
    });
    it('should create test configuration', () => {
        const config = globalThis.testUtils.createTestConfig();
        expect(config.environment).toBe('test');
        expect(config.memory.namespace).toBe('test-mcp-ocs');
    });
    it('should mock OpenShift responses', () => {
        const response = globalThis.testUtils.mockOcResponse('get pods', '{"items":[]}');
