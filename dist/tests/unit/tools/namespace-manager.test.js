 ...
 */
import { ToolNamespaceManager, ToolRegistry, } from '../../../src/lib/tools/namespace-manager';
describe('ToolNamespaceManager + ToolRegistry', () => {
    ...
    it('enables expected namespaces in single mode with cluster active', async () => {
        ...
        expect(enabled).toEqual(expect.arrayContaining(['mcp-core', 'mcp-files', 'mcp-memory', 'mcp-openshift']));
    ...
    it('flags conflicts via warning when conflicting tool already registered', async () => {
        ...
        const b = makeToolDef({ name: 'b', fullName: 'file_b', conflictsWith: ['file_a'] });
    ...
    it('filters tools by enabled namespaces and context requirements', async () => {
        ...
        const fileTool = makeToolDef({
            ...
            contextRequirements: [{ type: 'environment', value: 'test', required: true }],
        });
