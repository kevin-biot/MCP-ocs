...
describe('SharedMemoryManager (JSON mode)', () => {
    ...
    beforeEach(async () => {
        ...
        global.fetch = jest.fn().mockRejectedValue(new Error('no chroma'));
        restoreGlobals = () => { global.fetch = originalFetch; };
    ...
    it('returns stats with chromaAvailable false and storageUsed string', async () => {
        const spy = jest.spyOn(memory, 'calculateStorageUsage').mockResolvedValue('1 KB');
