...
describe('InfrastructureCorrelationChecker helpers', () => {
    ...
    beforeEach(() => {
        ...
        checker = new InfrastructureCorrelationChecker(oc, memory);
        const mocks = [];
...
describe('createInfrastructureCorrelationTools wrappers', () => {
    ...
    afterEach(() => { if (restoreConsole)
        restoreConsole(); jest.restoreAllMocks(); });
    it('returns success envelope on happy path', async () => {
        const spy = jest.spyOn(InfrastructureCorrelationChecker.prototype, 'checkInfrastructureCorrelation').mockResolvedValue({
        ...
        const tools = createInfrastructureCorrelationTools(oc, memory);
    ...
    it('returns error envelope on failure', async () => {
        jest.spyOn(InfrastructureCorrelationChecker.prototype, 'checkInfrastructureCorrelation').mockRejectedValue(new Error('nope'));
        const tools = createInfrastructureCorrelationTools(oc, memory);
